...
public class SmackImpl implements Smack {
	public static final String XMPP_IDENTITY_NAME = "shoudle";
	...
	private static final String PING_ALARM = "cn.shoudle.PING_ALARM";
	private static final String PONG_TIMEOUT_ALARM = "cn.shoudle.PONG_TIMEOUT_ALARM";
	...
	public SmackImpl(SdService service) {
		String server = SdConfig.SD_SERVER;
		boolean smackdebug = SdConfig.SMACK_DEBUG;
		boolean requireSsl = SdConfig.REQUIRE_SSL;
		...
		try {
			if (mXMPPConnection.isConnected()) {
				try {
					...
					SdLog.d(TAG,"conn.disconnect() failed: " + e);
				}
			}
			...
			if (!mXMPPConnection.isConnected()) {
				throw new SdException("SMACK connect failed without exception!");
			}
			...
			if (!mXMPPConnection.isAuthenticated()) {
				String ressource = PreferenceUtils.getPrefString(mService,SdConstants.CONS_RESSOURCE, "SD");
			...
			throw new SdException(e.getLocalizedMessage(),e.getWrappedThrowable());
		} catch (Exception e) {
			throw new SdException(e.getLocalizedMessage(), e.getCause());
		}
				...
				try {
					if (packet instanceof Message) {
						...
						if (cc != null
							...
							/*Sd* addChatMessageToDB(ChatConstants.OUTGOING, fromJID,
						...
						/*SD*addChatMessageToDB(ChatConstants.INCOMING, fromJID,
					...
					// processPacket :(
					SdLog.e(TAG,"failed to process packet:");
					e.printStackTrace();
				}
			}
		};
						...
						Log.d(TAG,
								"message "
										...
										+ (msg.getPacketID() == null ? "null"
												: msg.getPacketID()) + ")");
						/*Sd* changeMessageDeliveryStatus(msg.getPacketID(),
	...
	/*Sd* public void changeMessageDeliveryStatus(String packetID, int new_status) {
	...
	private class PongTimeoutAlarmReceiver extends BroadcastReceiver {
		public void onReceive(Context ctx, Intent i) {
			SdLog.d(TAG,"Ping: timeout for " + mPingID);
			mService.postConnectionFailed(SdService.PONG_TIMEOUT);
			logout();// 超时就断开连接
		}
	}
	...
	private class PingAlarmReceiver extends BroadcastReceiver {
		public void onReceive(Context ctx, Intent i) {
			if (mXMPPConnection.isAuthenticated()) {
				...
				SdLog.d(TAG,"Ping: alarm received, but not connected to server.");
		}
	}
	...
	/***************** start 发送离线消息 ***********************/
	/*Sd* public void sendOfflineMessages() {
	...
	private void registerRosterListener() {
		...
		mRosterListener = new RosterListener() {
			...
			public void presenceChanged(Presence presence) {
				SdLog.i(TAG,"presenceChanged(" + presence.getFrom() + "): " + presence);
			...
			public void entriesUpdated(Collection<String> entries) {
				...
				SdLog.i(TAG,"entriesUpdated(" + entries + ")");
			...
			public void entriesDeleted(Collection<String> entries) {
				SdLog.i(TAG,"entriesDeleted(" + entries + ")");
				for (String entry : entries) {
			...
			public void entriesAdded(Collection<String> entries) {
				SdLog.i(TAG,"entriesAdded(" + entries + ")");
	...
	private void addRosterEntryToDB(final RosterEntry entry) {
		...
		//SdLog.i(TAG,"addRosterEntryToDB: Inserted " + uri);
	}
	...
	private void deleteRosterEntryFromDB(final String jabberID) {
		...
		//SdLog.i(TAG,"deleteRosterEntryFromDB: Deleted " + count + " entries");
	}
...
/*	private StatusMode getStatus(Presence presence) {
	...
	private void initServiceDiscovery() {
		...
		dm.registerReceiptReceivedListener(new DeliveryReceiptManager.ReceiptReceivedListener() {
			public void onReceiptReceived(String fromJid, String toJid,
					String receiptId) {
				SdLog.d(TAG, "got delivery receipt for " + receiptId);
	...
	private void addRosterEntry(String user, String alias, String group)
		...
		try {
			...
			throw new SdException(e.getLocalizedMessage());
		}
	}
	...
	public void removeRosterItem(String user) throws SdException {
		// TODO Auto-generated method stub
		SdLog.d(TAG,"removeRosterItem(" + user + ")");

		removeRosterEntry(user);
		mService.rosterChanged();
	}

	private void removeRosterEntry(String user) throws SdException {
		mRoster = mXMPPConnection.getRoster();
		try {
	...
	public void renameRosterItem(String user, String newName)
		...
		if (!(newName.length() > 0) || (rosterEntry == null)) {
			throw new SdException("JabberID to rename is invalid!");
		}
	...
	private void tryToRemoveUserFromGroup(RosterGroup group,
			RosterEntry rosterEntry) throws SdException {
		try {
	...
	public void renameRosterGroup(String group, String newGroup) {
		// TODO Auto-generated method stub
		SdLog.i(TAG,"oldgroup=" + group + ", newgroup=" + newGroup);
		mRoster = mXMPPConnection.getRoster();
	...
	public void sendServerPing() {
		if (mPingID != null) {
			SdLog.d(TAG,"Ping: requested, but still waiting for " + mPingID);
			return; // a ping is still on its way
		}
		Ping ping = new Ping();
		ping.setType(Type.GET);
		ping.setTo(SdConfig.SD_SERVER);
		mPingID = ping.getPacketID();
		mPingTimestamp = System.currentTimeMillis();
		SdLog.d(TAG,"Ping: sending ping " + mPingID);
	...
	public boolean logout() {
		SdLog.d(TAG,"unRegisterCallback()");
		...
		if (mXMPPConnection.isConnected()) {
			...
			new Thread() {
				public void run() {
					SdLog.d(TAG,"shutDown thread started");
					mXMPPConnection.disconnect();
					SdLog.d(TAG,"shutDown thread finished");
				}
			}.start();
		}
		setStatusOffline();
